...
O repositório contém `functions/src/index.ts` e `functions/src/extractor.ts`, com callable autenticada `extractLinkMetadata`, parser JSON-LD/Open Graph/itemprop, fallback manual, limite por usuário, limite de concorrência, timeout, limite de bytes, redirects limitados e resolução DNS fixada após validação. O cliente Flutter usa a função por padrão quando Firebase, autenticação e endpoint estão disponíveis; `LUME_ENABLE_LINK_EXTRACTION=false` desativa a integração. A cobertura das Security Rules no Emulator Suite já está em `firebase/rules-tests`; deploy da Function no projeto `lume-13125`, teste integrado da callable, billing/alertas e testes com páginas reais continuam sendo gates operacionais antes de usar a extração em produção.
